...
@task
def start_jupyter(c, env: str = 'singularity', partition: str = 'xenon1t', 
            bypass_reservation: bool = False, tag: str = 'development',
            node: str = None, timeout: int = 120, cpu: int = 2, ram: int = 8000,
            gpu: bool = False, jupyter: str = 'lab', local_cutax: bool = False,
            notebook_dir: str = None, max_hours: int = 4, force_new: bool = False,
            local_port=8889, detached: bool = False):
    ...
    if env == 'singularity':
        ...
        batch_job = JOB_HEADER + \
                    "{starter_path} " \
                    "{s_container} {jupyter} {nbook_dir}".format(starter_path=starter_path,
        ...
        batch_job = (JOB_HEADER
                     ...
                     + START_JUPYTER.format(jupyter=jupyter,
    ...
    _want_to_make_reservation = (partition == 'xenon1t'
                                    and (not bypass_reservation))
    if ram > 16000 and _want_to_make_reservation:
        print('You asked for more than 16 GB total memory you cannot use the notebook '
                    'reservation queue for this job! We will bypass the reservation.')

    if cpu >= 8 and _want_to_make_reservation:
        print('You asked for more than 7 CPUs you cannot use the notebook reservation '
    ...
    if use_reservation:
        with c.console.status('Notebook reservation requested. Checking availability...'):
    ...
    extra_header = (GPU_HEADER if gpu
                    else CPU_HEADER.format(partition=partition,
                                            qos=qos,
    ...
    with c.console.status("Waiting for jupyter server to start...") as status:
        ...
        for _ in range(timeout):
            ...
            for line in lines:
                ...
                df = parse_squeue_output(result.stdout)
                if not len(df):
                    raise RuntimeError("Job has exited."
                                    "This can be because it was canceled or due to an internal error.")
                continue
            break
        else:
            raise RuntimeError("Jupyter server did not start within %d seconds." % timeout)
    ...
    with c.console.status(msg) as status:
        ...
        if detached:
            c.local(f"ssh -fN -L {local_port}:{remote_host}:{remote_port} {c.user}@{c.host} &",
                    disown=True, hide=False, warn=True)
            time.sleep(3)
            ...
            with c.forward_local(local_port, remote_port=remote_port, remote_host=remote_host):
                status.update("Local port forwarding is active.\n"
                              "   Press ENTER or CTRL-C to deactivate and cancel job.")
                time.sleep(3)
                result = c.local(f"python -m webbrowser -t {local_url}", hide=True, warn=True)
                try:
                    c.console.input()
